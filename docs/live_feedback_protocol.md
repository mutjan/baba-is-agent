...
`baba_search_route.py --from-live-state --time-limit 5` 默认固定现有 YOU 文字；只有明确需要时加 `--move-you-text`。用 `--target-start X,Y --target-dir right` 缩小目标位置。统一协作式预算覆盖模式、分配、启发式、可达区域与求解；不是操作系统级硬实时截止。每秒 stderr 报阶段和计数。时间或规模耗尽返回退出码 3、`budget_exhausted`，不代表无解。
...
```
python3 scripts/baba_action_check.py 'right*2' --expect-moved-delta baba:+x --action-id my-action
